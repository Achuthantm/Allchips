...
class MCCFRTrainer:
    ...
    def get_hand_bucket(self, hand, board, round_idx):
        ...
        # Cache HS calculation
        key = (tuple(sorted([str(c) for c in hand])), tuple(sorted([str(c) for c in board])))
        if key in self.hs_cache:
    ...
    def is_round_over(self, history, round_idx, p1_bet, p2_bet):
    ...
    def mccfr(self, p1_hand, p2_hand, board, history, round_idx, p1_bet, p2_bet, active_player, deck, p1_bucket, p2_bucket, traversing_player):
        ...
        if self.is_round_over(history, round_idx, p1_bet, p2_bet):
        ...
        if active_player == traversing_player:
            ...
            if active_player == 1: regrets = -regrets
            node.regret_sum += regrets
            return util
        else: # Sample other player
